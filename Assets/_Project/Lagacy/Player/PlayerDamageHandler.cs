...
/// </summary>
public class PlayerDamageHandler : MonoBehaviour, ILegacyDamageable
{
    // 추후 PlayerVFXManager, PlayerAnimator 등이 구독할 피격 방송 채널
    public event Action<LegacyDamageInfo> OnTakeDamageEvent;

    // 정수 피해를 기존 피해 정보로 변환해 처리
    public void TakeDamage(int amount)
    {
        // 간단한 int 입력을 기존 피해 정보로 포장하고 기본 무적 1초 부여
        LegacyDamageInfo info = new LegacyDamageInfo()
        {
            Amount = amount,
            InvincibleTime = 1.0f,
