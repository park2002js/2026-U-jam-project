...
{
    /// <summary>가시 탄환: 슈팅에 맞은 적에게 확률적으로 Bleed 속성을 부여한다.</summary>
    public sealed class ThornBulletEffect : ItemEffect
    {
        private readonly float chance;
        /// <summary>Catalog에서 0~100의 발동 확률을 지정한다.</summary>
        public ThornBulletEffect(float chance) => this.chance = Mathf.Clamp(chance, 0, 100);
        ...
        public override void Execute(ItemUseContext c)
        {
            ...
            foreach (var enemy in c.Enemies)
            {
                ...
                c.Element(enemy, ItemElement.Bleed);
                /* VFX: 가시 탄환의 속성 적중 표현을 이곳에 추가한다. */
            }
        }
    }
}
